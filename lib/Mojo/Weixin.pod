...
需要依赖模块 L<Mojo::IRC::Server::Chinese>
    ...
    allow_group         # allow_group=>["群组名称1","群组名称2",], #可选，允许在irc上创建的微信群名称
    ban_group           # ban_group=>["群组名称1","群组名称2"],  #可选，禁止在irc上创建的微信群名称
    master_irc_nick     #和微信匹配的irc帐号昵称，默认按照和微信号相同的昵称或者客户端ip是本机地址作为识别规则
